...
/obj/structure/shuttle_decoration/liquid_tank/coolant/industrial
	...
	icon = 'modular_aphelion/modules/shipbreaking/icons/closet.dmi'
...
/**
 * Starts the second meltdown stage, with arcing and upset sounds
 *
 * Arms the final stage timer. Does nothing if the reactor was already
 * shipped off on the supply shuttle.
 */
...
/**
 * Starts the final meltdown stage, where the reactor can explode at any moment
 *
 * Alerts admins and ghosts. Does nothing if the reactor was already
 * shipped off on the supply shuttle.
 */
...
/obj/structure/shuttle_decoration/liquid_tank/reactor/process(seconds_per_tick)
	...
	if((meltdown_state >= REACTOR_MELTDOWN_VERY_UPSET) && SPT_PROB(1, seconds_per_tick))
...
/obj/structure/shuttle_decoration/liquid_tank/reactor/rupture_tank()
	playsound(src, 'modular_aphelion/modules/shipbreaking/sound/plasma_bomb.ogg', 100, FALSE, 70, 1, pressure_affected = FALSE, ignore_walls = TRUE)
...
/obj/structure/shuttle_decoration/liquid_tank/reactor/super/rupture_tank()
	playsound(src, 'modular_aphelion/modules/shipbreaking/sound/plasma_bomb.ogg', 100, FALSE, 100, 1, pressure_affected = FALSE, ignore_walls = TRUE)
...
#undef REACTOR_MELTDOWN_SHUTTLE_DISARMED
