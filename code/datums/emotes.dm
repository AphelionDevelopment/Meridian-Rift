	...
	if(tmp_sound && should_play_sound(user, intentional))
		...
		*/ // NOVA EDIT REMOVAL END
		...
		if(running_emote_type & EMOTE_LEWD)
			playsound_if_pref(source = user,soundin = tmp_sound,vol = sound_volume, vary = FALSE, ignore_walls = sound_wall_ignore, frequency = frequency, pref_to_check = /datum/preference/toggle/erp/sounds)
