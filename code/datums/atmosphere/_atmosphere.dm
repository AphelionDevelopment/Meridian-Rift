...
/datum/atmosphere/proc/generate_gas_string()
	...
	target_pressure = clamp(target_pressure, minimum_pressure + (gasmix.get_moles(gastype) * 0.1), maximum_pressure)
	...
	for(var/gas_id, gas_amount in gasmix.get_moles_list())
