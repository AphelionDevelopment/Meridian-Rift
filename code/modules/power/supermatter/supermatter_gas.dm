...
/datum/sm_gas/carbon_dioxide/extra_effects(obj/machinery/power/supermatter_crystal/sm)
	...
	var/consumed_co2 = sm.absorbed_gasmix.get_moles(/datum/gas/carbon_dioxide) * co2_ratio
	consumed_co2 = min(
		consumed_co2,
		sm.absorbed_gasmix.get_moles(/datum/gas/carbon_dioxide),
		sm.absorbed_gasmix.get_moles(/datum/gas/oxygen)
	)
	if(!consumed_co2)
		return
...
/datum/sm_gas/miasma/extra_effects(obj/machinery/power/supermatter_crystal/sm)
	...
	var/consumed_miasma = sm.absorbed_gasmix.get_moles(/datum/gas/miasma) * miasma_ratio
	if(!consumed_miasma)
		return
