...
/mob/living/can_perform_action(atom/target, action_bitflags)
	...
	if(!(action_bitflags & BYPASS_ADJACENCY) && ((action_bitflags & NOT_INSIDE_TARGET) || !recursive_loc_check(src, target)) && !target.IsReachableBy(src))
		if(HAS_SILICON_ACCESS(src) && !ispAI(src))
			...
			if(!can_psionically_reach(target) && (!mob_DNA || !mob_DNA.check_mutation(/datum/mutation/telekinesis) || !tkMaxRangeCheck(src, target))) // APHELION EDIT CHANGE - PSIONICS - ORIGINAL: if(!mob_DNA || !mob_DNA.check_mutation(/datum/mutation/telekinesis) || !tkMaxRangeCheck(src, target))
				if(!(action_bitflags & SILENT_ADJACENCY))
					to_chat(src, span_warning("You are too far away!"))
				return FALSE
