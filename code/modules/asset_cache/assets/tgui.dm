...
// APHELION EDIT ADDITION START - LOBBY_MENU_REWORK - ships Aphelion's own Fixedsys lobby font instead, since the lobby menu is reskinned to Nova's plain-text look (see tgui-lobby/styles/fonts.css).
// Parent key is "lobby-fonts.css", not the generic "fonts.css" upstream used - that literal
// name collides with /datum/asset/simple/namespaced/escape_menu_font's own "fonts.css" parent
// in the shared SSassets.cache namespace (see /datum/asset_transport/proc/register_asset),
// which logs an asset-cache error whenever both get registered in the same server session.
/datum/asset/simple/namespaced/lobby_menu_font
	assets = list(
		"FixedsysExcelsior3.01Regular.ttf" = file("html/browser/FixedsysExcelsior3.01Regular.ttf"),
	)
	parents = list(
		"lobby-fonts.css" = file("tgui/packages/tgui-lobby/styles/fonts.css"),
	)
