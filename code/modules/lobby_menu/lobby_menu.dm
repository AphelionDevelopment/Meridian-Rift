...
/* // APHELION EDIT REMOVAL START - LOBBY_MENU_REWORK - moved into modular_nova/modules/title_screen/code/title_screen_subsystem.dm's show_title_screen(), which now (re-)registers this asset itself whenever the title image changes, instead of once via SStitle.icon
...
/client/proc/initialize_lobby_menu()
	set waitfor = FALSE
	//register_lobby_title_asset() // APHELION EDIT REMOVAL - LOBBY_MENU_REWORK - SStitle.show_title_screen() now registers this asset itself during subsystem Initialize(), well before any client can connect
	// Wait for the title screen asset to be available
	UNTIL(SStitle.current_title_asset_name) // APHELION EDIT CHANGE - LOBBY_MENU_REWORK - ORIGINAL: UNTIL(SSassets.transport.get_asset_url(LOBBY_TITLE_ASSET_NAME))
	if(!src)
		return
	lobby_menu = new(src)
...
/datum/lobby_menu/proc/initialize_browser()
	...
	SSassets.transport.send_assets(client, SStitle.current_title_asset_name) // APHELION EDIT CHANGE - LOBBY_MENU_REWORK - ORIGINAL: SSassets.transport.send_assets(client, LOBBY_TITLE_ASSET_NAME)
...
/datum/lobby_menu/proc/update_visibility()
	...
	winset(client, SKIN_MAPWINDOW_STATUS_BAR, "is-visible=[!should_show]") // APHELION EDIT ADDITION - LOBBY_MENU_REWORK - hide the ping/status corner behind the lobby
	if(should_show)
	...
	switch(action)
		if("ready_toggle")
			...
			if(SSticker && SSticker.current_state > GAME_STATE_PREGAME)
				to_chat(player, span_notice("It's too late for that, the round is already starting!"))
				return TRUE
			if(player.ready == PLAYER_NOT_READY && CONFIG_GET(flag/min_flavor_text) && !is_admin(client))
				var/flavor_length = length_char(client?.prefs?.read_preference(/datum/preference/text/flavor_text))
				if(flavor_length < CONFIG_GET(number/flavor_text_character_requirement))
					to_chat(player, span_notice("You need at least [CONFIG_GET(number/flavor_text_character_requirement)] characters of flavor text to ready up for the round. You have [flavor_length] characters."))
					return TRUE
			// APHELION EDIT ADDITION END
			if(player.ready == PLAYER_NOT_READY)
				player.auto_deadmin_on_ready_or_latejoin()
				player.ready = PLAYER_READY_TO_PLAY
				SSstatpanels.add_job_estimation(player) // APHELION EDIT ADDITION - LOBBY_MENU_REWORK
			else
				player.ready = PLAYER_NOT_READY
				SSstatpanels.remove_job_estimation(player) // APHELION EDIT ADDITION - LOBBY_MENU_REWORK
		...
		if("server_swap")
			player.server_swap()
		if("view_directory")
			INVOKE_GAME_VERB(client, player, /client, show_character_directory)
		if("toggle_antag")
			...
			send_update(list("isAntag" = preferences.read_preference(/datum/preference/toggle/be_antag)))
		// APHELION EDIT ADDITION END
		if("start_now")
