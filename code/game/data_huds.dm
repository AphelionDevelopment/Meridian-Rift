...
/proc/RoundHealth(mob/living/M)
	...
	var/resulthealth = M.get_health_hud_percent()
	switch(resulthealth)
		if(100 to INFINITY)
			return "health100"
		if(90.625 to 100)
...
/mob/living/carbon/human/proc/update_ID_card()
	...
	if(permit_icon_state)
		set_hud_image_state(PERMIT_HUD, hud_icon = MODULAR_HUDS_DMI, hud_state = permit_icon_state)
	else
		set_hud_image_state(PERMIT_HUD, hud_state = "hudfan_no")
