...
/obj/item/pet_carrier/interact_with_atom(atom/interacting_with, mob/living/user, list/modifiers)
	...
	if(target.mob_size > max_occupant_weight)
		if(ishuman(target))
			if(HAS_TRAIT(target, TRAIT_CATLIKE_INSTINCT) || isfeline(target)) // APHELION EDIT CHANGE - isfeline kept for Nova's feline species/quirk - ORIGINAL: if(HAS_TRAIT(target, TRAIT_CATLIKE_INSTINCT))
