...
// What the plate stops is non-penetrating: it spikes pain, bruises or burns the body underneath,
// wears the plate out, and never reaches anything inside. What gets past behaves as an unarmoured hit.

/// Damage a plate stops outright, per level. Level 1 stops 10, level 2 stops 20, and so on.
#define PLATE_TOLERANCE_PER_LEVEL 10
/// Damage a plate absorbs before it stops working, per level. Roughly ten hits at its own tolerance,
/// and far more against hits well under it.
#define PLATE_DURABILITY_PER_LEVEL 100

/// Portion of a stopped hit that lands as temporary pain. Under [PAIN_IMPACT_RATIO].
...
/// How long fitting a plate into a carrier, or prying one back out, takes.
#define PLATE_FITTING_TIME (4 SECONDS)

// How worn a plate is, as a portion of its durability. Used for examine text and wear feedback only;
// a plate works identically at every band until it is spent.
/// Marked only.
