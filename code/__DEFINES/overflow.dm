...
// penetrating damage lands on whatever the part was protecting, which is overflow. A finisher is the
// deliberate version: a telegraphed execution on a helpless target with every condition checked.
...
/// the part's own overflow target takes. Lighter than the kill route.
#define OVERFLOW_ORGAN_DAMAGE_RATIO 0.5

/// Least damage a penetrating hit needs before it takes a limb off. Heads and chests deliberately
/// have no equivalent floor: what reaches the organ is a proportion of the hit, so a graze stays a
/// graze and sustained burning can still reach a heart.
...
/// regardless. Sits above the security baton deliberately.
