...
#define GET_PHYSIOLOGY(living, key) (isnull(LAZYACCESS(living.physiology, key)) ? DEFAULT_PHYSIOLOGY_VAL : living.physiology[key]) // APHELEON EDIT ORIGINAL: #define GET_PHYSIOLOGY(living, key) (LAZYACCESS(living.physiology, key) || DEFAULT_PHYSIOLOGY_VAL)
