//Modular Undershirts
/datum/sprite_accessory/clothing/undershirt
	use_static = TRUE
	layer = UNDER_UNIFORM_LAYER
	///Whether this underwear includes a bottom (For Leotards and the likes)
	var/hides_groin = FALSE
/* Base recolorable shirts
*/
/datum/sprite_accessory/clothing/undershirt/shirt
	name = "Shirt"
	icon_state = "shirt_white" //Reuses TG sprite until they set up GAGS for underwear
	use_static = FALSE

/datum/sprite_accessory/clothing/undershirt/shortsleeve
	name = "Short-Sleeved Shirt"
	icon_state = "whiteshortsleeve" //Reuses TG sprite until they set up GAGS for underwear
	use_static = FALSE

/datum/sprite_accessory/clothing/undershirt/tanktop
	name = "Tank Top"
	icon_state = "tank_white" //Reuses TG sprite until they set up GAGS for underwear
	use_static = FALSE

/datum/sprite_accessory/clothing/undershirt/tanktop_midriff
	name = "Tank Top - Midriff"
	icon_state = "tank_midriff"
	gender = FEMALE
	use_static = FALSE

/datum/sprite_accessory/clothing/undershirt/tankstripe
	name = "Tank Top - Striped"
	icon_state = "tank_stripes"

/* Nova unique shirts
*/
/datum/sprite_accessory/clothing/undershirt/nova
	icon = 'modular_nova/master_files/icons/mob/clothing/underwear.dmi'
	abstract_type = /datum/sprite_accessory/clothing/undershirt/nova


/datum/sprite_accessory/clothing/undershirt/nova/longsleeve
	name = "Long-Sleeved Shirt"
	icon_state = "shirt_white_long"
	use_static = FALSE

/datum/sprite_accessory/clothing/undershirt/nova/polo
	name = "Polo Shirt"
	icon_state = "polo"
	use_static = FALSE

/datum/sprite_accessory/clothing/undershirt/nova/tanktop_midriff_alt
	name = "Tank Top - Midriff Halterneck"
	icon_state = "tank_midriff_alt"
	gender = FEMALE
	use_static = FALSE

/datum/sprite_accessory/clothing/undershirt/nova/offshoulder
	name = "Shirt - Off-Shoulder"
	icon_state = "one_arm"
	gender = FEMALE
	use_static = FALSE

/datum/sprite_accessory/clothing/undershirt/nova/buttondown
	name = "Shirt - Buttondown"
	icon_state = "buttondown"
	gender = NEUTER
	use_static = FALSE

/datum/sprite_accessory/clothing/undershirt/nova/buttondown/short_sleeve
	name = "Shirt - Short Sleeved Buttondown"
	icon_state = "buttondown_short"

/datum/sprite_accessory/clothing/undershirt/nova/leotard
	name = "Shirt - Leotard"
	icon_state = "leotard"
	gender = FEMALE
	use_static = FALSE
	hides_groin = TRUE

/datum/sprite_accessory/clothing/undershirt/nova/turtleneck
	name = "Sweater - Turtleneck"
	icon_state = "turtleneck"
	use_static = FALSE
	gender = NEUTER

/datum/sprite_accessory/clothing/undershirt/nova/turtleneck/smooth
	name = "Sweater - Smooth Turtleneck"
	icon_state = "turtleneck_smooth"

/datum/sprite_accessory/clothing/undershirt/nova/turtleneck/sleeveless
	name = "Sweater - Sleeveless Turtleneck"
	icon_state = "turtleneck_sleeveless"

/datum/sprite_accessory/clothing/undershirt/nova/leotard/turtleneck
	name = "Shirt - Turtleneck Leotard"
	icon_state = "leotard_turtleneck"

/datum/sprite_accessory/clothing/undershirt/nova/leotard/turtleneck/sleeveless
	name = "Shirt - Turtleneck Leotard Sleeveless"
	icon_state = "leotard_turtleneck_sleeveless"

//Presets
/datum/sprite_accessory/clothing/undershirt/nova/bulletclub //4 life
	name = "Shirt - Black Skull"
	icon_state = "shirt_bc"
	gender = NEUTER

/datum/sprite_accessory/clothing/undershirt/nova/bee_shirt
	name = "Shirt - Bee"
	icon_state = "bee_shirt"

/datum/sprite_accessory/clothing/undershirt/nova/striped
	name = "Long-Sleeved Shirt - Black Stripes"
	icon_state = "longstripe"
	gender = NEUTER

/datum/sprite_accessory/clothing/undershirt/nova/striped/blue
	name = "Long-Sleeved Shirt - Blue Stripes"
	icon_state = "longstripe_blue"

/datum/sprite_accessory/clothing/undershirt/nova/tank_top_rainbow
	name = "Tank Top - Rainbow"
	icon_state = "tank_rainbow"

/datum/sprite_accessory/clothing/undershirt/nova/tank_top_sun
	name = "Tank Top - Sun"
	icon_state = "tank_sun"

//Not really qualifying as shirts but having nowhere better to go, these get shoved to the bottom of the list
/datum/sprite_accessory/clothing/undershirt/nova/corset
	name = "Corset"
	icon_state = "corset"
	gender = FEMALE
	hides_groin = TRUE //an undershirt-specific bit of code, so the corset has to be an undershirt... unless you want to refactor it

//This was going to be a bra, then I realized that bras do not get chopped apart by male/female bodytypes. I guess it makes more sense as an undershirt?
/datum/sprite_accessory/clothing/undershirt/nova/crop_corset
	name = "Corset - Cropped"
	icon_state = "crop_corset"
	use_static = FALSE
	gender = FEMALE

/datum/sprite_accessory/clothing/undershirt/nova/babydoll
	name = "Babydoll"
	icon_state = "babydoll"
	gender = FEMALE
	use_static = FALSE

//These are just copies of a bra sprite-accessory, but they layer over other bras instead.
/datum/sprite_accessory/clothing/undershirt/nova/fishnet_sleeves
	name = "Fishnet - Sleeved (Greyscale)"
	icon_state = "fishnet_sleeves_alt"
	use_static = FALSE

/datum/sprite_accessory/clothing/undershirt/nova/fishnet_base
	name = "Fishnet - Sleeveless (Greyscale)"
	icon_state = "fishnet_body_alt"
	use_static = FALSE
