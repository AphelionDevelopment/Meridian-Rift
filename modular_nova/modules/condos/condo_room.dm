...
/datum/condo_room
	...
	var/display_name
	/// Account ckey of the room owner.
	var/owner_ckey
	/// Character name of the room owner.
	var/owner_name
	...
	/// Map template used to populate the room.
	var/datum/map_template/condo/template
	/// Turf reservation containing the loaded room.
	var/datum/turf_reservation/condo/reservation
	/// Lighting/ambience choices, re-appliable and editable from the door panel.
	/// Brightness level: 0 = off, 1 = dim, 2 = normal, 3 = bright.
	var/brightness = 2
	/// Optional six-digit hexadecimal lamp color.
	var/lamp_color
	/// Key of the selected ambience preset.
	var/ambience
