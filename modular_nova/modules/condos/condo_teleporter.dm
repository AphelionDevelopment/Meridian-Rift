...
#define CONDO_MAX_ROOMS_PER_OWNER 3
#define CONDO_MAX_ACTIVE_ROOMS 50
...
/obj/machinery/cafe_condo_teleporter/ui_act(action, list/params, datum/tgui/ui)
	...
	switch(action)
		...
		if("create")
			...
			var/owned_rooms = 0
			for(var/room_id in SScondos.active_condos)
				var/datum/condo_room/active_room = SScondos.active_condos[room_id]
				if(active_room.owner_ckey == user.ckey)
					owned_rooms++
			if(owned_rooms >= CONDO_MAX_ROOMS_PER_OWNER)
				to_chat(user, span_warning("You already own the maximum number of condo rooms."))
				return TRUE
			if(length(SScondos.active_condos) >= CONDO_MAX_ACTIVE_ROOMS)
				to_chat(user, span_warning("The maximum number of active condo rooms has been reached."))
				return TRUE
			var/private = params["private"]
			var/room_name = sanitize(params["name"])
			var/datum/condo_room/room = SScondos.create_room(chosen_condo, user, src, room_name, private, private ? copytext(params["password"], 1, 43) : null)
			if(!room)
				return TRUE
...
#undef CONDO_MAX_ROOMS_PER_OWNER
#undef CONDO_MAX_ACTIVE_ROOMS
