/datum/design/biogen/frontier_ration
	name = "Frontier Ration Basetype"
	abstract_type = /datum/design/biogen/frontier_ration
	build_path = /obj/item/reagent_containers/condiment
	materials = list(/datum/material/biomass = 100)
	category = list(
		RND_CATEGORY_INITIAL,
		RND_CATEGORY_AKHTER_FOODRICATOR_BAGS,
	)

/datum/design/biogen/frontier_ration/flour
	name = "Flour Sack"
	build_path = /obj/item/reagent_containers/condiment/flour/small_ration

/datum/design/biogen/frontier_ration/flour_korta
	name = "Korta Flour Sack"
	build_path = /obj/item/reagent_containers/condiment/small_ration_korta_flour

/datum/design/biogen/frontier_ration/rice
	name = "Rice Sack"
	build_path = /obj/item/reagent_containers/condiment/rice/small_ration

/datum/design/biogen/frontier_ration/sugar
	name = "Sugar Sack"
	build_path = /obj/item/reagent_containers/condiment/sugar/small_ration

/datum/design/biogen/frontier_ration/soy_milk
	name = "Soy Milk"
	build_path = /obj/item/reagent_containers/condiment/soymilk/small_ration

/datum/design/biogen/frontier_ration/milk
	name = "Milk"
	build_path = /obj/item/reagent_containers/condiment/milk/small_ration
