...
/obj/item/blowing_horn/attack_self(mob/living/user)
	if (user.get_sprint_stamina_loss() > BHORN_STAMINA_MINIMUM) // NOVA EDIT CHANGE - ORIGINAL: if (user.get_stamina_loss() > BHORN_STAMINA_MINIMUM)
	...
	user.adjust_sprint_stamina_loss(BHORN_STAMINA_USE) // NOVA EDIT CHANGE - ORIGINAL: user.adjust_stamina_loss(BHORN_STAMINA_USE)
...
/obj/structure/war_horn/attack_hand(mob/living/user)
	if (!ishuman(user))
		...
		return
	if (user.get_sprint_stamina_loss() > WHORN_STAMINA_MINIMUM) // NOVA EDIT CHANGE - ORIGINAL: if (user.get_stamina_loss() > WHORN_STAMINA_MINIMUM)
		balloon_alert(user, "too tired!")
		return
	if (user.is_mouth_covered())
		balloon_alert(user, "something is in the way!")
	...
	user.adjust_sprint_stamina_loss(WHORN_STAMINA_USE) // NOVA EDIT CHANGE - ORIGINAL: user.adjust_stamina_loss(WHORN_STAMINA_USE)
