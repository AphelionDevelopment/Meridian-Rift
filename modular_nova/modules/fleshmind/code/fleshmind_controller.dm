...
/datum/fleshmind_controller/proc/level_up()
	...
	switch(level)
		if(CONTROLLER_LEVEL_3)
			if(!tyrant_spawned)
				minor_announce("This is [controller_firstname], wirenet efficency has reached a point of singularity, initiating Protocol 34-C.", "[controller_fullname]: OFFENSIVE MEASURES ACTIVATED", sound_override = 'modular_nova/modules/fleshmind/sound/ai/tyrant.ogg')
		...
		if(CONTROLLER_LEVEL_4)
			minor_announce("This is [controller_firstname], kernel integrity is reaching the optimal conversion level.", "[controller_fullname]: Level [level]", sound_override = 'modular_nova/modules/fleshmind/sound/ai/data_compromised.ogg')
...
/datum/fleshmind_controller/proc/level_down()
	...
	if(level > 0)
		minor_announce("KERNEL INTEGRITY FALTERING. DO BETTER!", "[controller_fullname]: Level [level]", sound_override = pick('modular_nova/modules/fleshmind/sound/ai/level_down_1.ogg', 'modular_nova/modules/fleshmind/sound/ai/level_down_2.ogg', 'modular_nova/modules/fleshmind/sound/ai/level_down_3.ogg'))
		COOLDOWN_START(src, level_up_cooldown, FLESHCORE_LEVEL_UP_COOLDOWN)
	else
