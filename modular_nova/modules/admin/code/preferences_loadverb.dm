ADMIN_VERB(import_preferences, R_ADMIN, "Import Preferences", "Upload a character preferences JSON file to the server.", ADMIN_CATEGORY_MAIN)
	...
	// APHELION EDIT ADDITION - same sanitiser as the player-facing import
	// Everything above checks the container, not the values.
	// pass1 also flags the file so their next login rebuilds prefs.
