...
The Meridian-MCP integration qualified during the 2026-08-31 controller implementation exposed `rift_compile` only when configured with `MERIDIAN_MCP_RIFT_BUILD=offline|network` at server startup. Verify the installed server's exposed schema and configuration before relying on this historical interface. That integration runs this checkout's qualified root `RIFT_BUILD.cmd`, which accepts no arguments and delegates to `RIFT.cmd compile --mode full`. Caller `network_mode` is `offline` by default or `allow` under the configured network ceiling. `force_rebuild=true` removes only canonical compiled artifacts immediately before the build and establishes fresh artifacts for managed provenance; an unchanged build without a valid record can return insufficient evidence even when RIFT itself exits zero.
...
Humans continue to use `BUILD.cmd`. Agents must not modify that human entry point, `RUN_SERVER.cmd`, or inherited critical build/bootstrap infrastructure without naming exact files/effects and obtaining explicit confirmation unless that change is already authorized in the session.
